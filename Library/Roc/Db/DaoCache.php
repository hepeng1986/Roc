...
class Db_DaoCache
{
     ...
     * @return mixed
     */
    protected static function _formatSQL ($sSQL, $aParam)
    {
        if (! empty($aParam)) {
    ...
    {
        return self::query("SELECT * FROM " . self::getTable() . " WHERE " . self::getPKField() . "='$iPKID'", 'row');
    }
    ...
    public static function getPKIDList ($aPKIDs, $bUsePKID = false)
    {
        ...
        return self::query("SELECT * FROM " . self::getTable() . " WHERE " . self::getPKField() . " IN($sIDs)", 'all', $sField);
    }
    ...
    {
        return self::query(self::_buildSQL($aParam), 'all', $sField);
    }
    ...
    {
        return self::query(self::_buildSQL($aParam, 'COUNT(*)'), 'one');
    }
    ...
    {
        return self::query(self::_buildSQL($aParam, $sField, null, 1), 'one');
    }
    ...
    {
        return self::query(self::_buildSQL($aParam, $sField), 'col');
    }
    ...
    public static function getPair ($aParam, $sKeyField = null, $sValueField = null)
    {
        ...
        return self::query(self::_buildSQL($aParam, $sField), 'pair');
    }
    ...
    {
        return self::query(self::_buildSQL($aParam, $sFields, null, 1), 'row');
    }
    ...
    public static function query ($sSQL, $sType = 'query', $sField = null, $bCache = false)
    {
        ...
        //暂时先都不实用redis，等数据问题处理完稳定后再开启 add by zfx 20190521
        return self::queryData($sMethod, $sSQL, $sField);


        //判断是否开启了缓存
        if ((Yaf_G::getEnv() == 'ga') && defined($class . '::WHERE_CACHE') && $class::WHERE_CACHE && (substr(strtoupper(trim($sSQL)), 0, 6) == 'SELECT')) {
            $oCache = self::getCache();
            $sCacheKey = self::$DB_NAME . ':' . $class . ':' . md5($sSQL);

            $aRet = $oCache->get($sCacheKey);
            if (empty($aRet)) {
                $aRet = self::queryData($sMethod, $sSQL, $sField);
                $iExpire = defined($class . '::WHERE_CACHE') ? $class::CACHE_TIME : self::CACHE_TIME;
                $oCache->set($sCacheKey, $aRet, $iExpire);
            } else {
                Util_Common::addDebug('redis_sql', $sSQL);
            }
            return $aRet;
        } else {
            return self::queryData($sMethod, $sSQL, $sField);
     ...
     *  权限参数来读取数据
     */
    private static function queryData($sMethod, $sSQL, $sField) {
        if (!empty($sField)) {
