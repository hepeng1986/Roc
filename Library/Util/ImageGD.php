...
class Util_ImageGD
{
    ...
    protected function waterMark($p_oImg)
    {
        ...
        switch($this->_sWaterMarkPosition) {
            ...
            //右下
            case "bottom-right":
                $iDesX = $iImgWidth - $iWaterMarkWidth - 8;
                $iDesY = $iImgHeight - $iWaterMarkHeight - 8;
                break;
            //中下
            case "bottom-middle":
     ...
     * @return resource
     */
    public function resize($p_oImg, $p_iWidth, $p_iHeight, $p_bCrop = true, $p_WaterMark = true, $bFixedWater = false )
    {
        ...
        if (0 == $p_iWidth && 0 == $p_iHeight) {
            $oDesImg = imagecreatetruecolor($iOriginalWidth, $iOriginalWidth);

            $alpha = imagecolorallocatealpha($oDesImg, 0, 0, 0, 127);
            imagefill($oDesImg, 0, 0, $alpha);
            imagecopyresampled($oDesImg, $p_oImg, 0, 0, 0, 0,
                $iOriginalWidth, $iOriginalWidth,
                $iOriginalWidth, $iOriginalHeight
            );
            imagesavealpha($oDesImg, true);
            imagedestroy($p_oImg);
...
//            $oDesImg = $p_oImg;
        ...
        if ($bFixedWater || $this->chkWaterMark($oDesImg) && $p_WaterMark) {
     ...
     */
    public function createTypeImg($p_oImg, $p_sType = 'image/jpeg', $p_sFileName = null, $iImageQuality=100)
    {
        ...
        if ($p_sFileName) {
            if ($sTypeImageFunc == 'imagejpeg') {
                $sTypeImageFunc($p_oImg, $p_sFileName, $iImageQuality);
            } else {
            ...
            if ($sTypeImageFunc == 'imagejpeg') {
                $sTypeImageFunc($p_oImg, null, $iImageQuality);
            } else {
     ...
     * @return string
     */
    public function getImageBlob($p_oImg, $p_sType = 'image/jpeg', $iImageQuality=100)
    {
        $sTmpfile = tempnam("/dev/shm/", "phpgd_");
        $this->createTypeImg($p_oImg, $p_sType, $sTmpfile, $iImageQuality);
