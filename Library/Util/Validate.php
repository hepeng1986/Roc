...
class Util_Validate
{
    ...
    public static function isMonthFormat ($date)
    {
        return (bool) preg_match('/^([0-9]{4})-((0?[0-9])|(1[0-2]))$/', $date);
    }
...
//        return preg_match('/^https?:\/\/[!,:#%&_=\(\)\.\? \+\-@\/a-zA-Z0-9]+$/', $url);
    ...
    /*
     * 校验时间 兼容3种形式  2018,2018-01,2018-01-01
     */
    public static function isYmd($data){
        $bRet = false;
        //年
        if(preg_match('/^([0-9]{4})$/', $data)){
            return true;
        }elseif(preg_match('/^([0-9]{4})-(0[1-9]|1[0-2])$/', $data)){
            return true;
        }elseif (preg_match('/^([0-9]{4})-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[01])$/',$data,$matches)){
            return checkdate(intval($matches[2]), intval($matches[3]), intval($matches[1]));
        }
        return $bRet;
    }
    /*
     * 校验时间 兼容2种形式  2018,2018-01
     */
    public static function isYmonth($data){
        $bRet = false;
        //年
        if(preg_match('/^([0-9]{4})$/', $data)){
            return true;
